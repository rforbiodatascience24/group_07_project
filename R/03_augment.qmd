...
pheno_data_filter <- pheno_data |>
  rename(sample = geo_accession,
         ...
         sex = `sex:ch1`,
         tissue = `tissue:ch1`) |>
  select(sample, organism, age, sex, tissue)
```

We also change the observations of new 'Age' column to only contain number.
...
pheno_expr_data_joined <- full_join(expr_data_augment,
                                    pheno_data_augment,
                                    by = "sample")
```
