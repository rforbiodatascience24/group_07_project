...
First, we recreate the plots from the report, however, since we only have data for a single tissue, we just replicate the \_\_\_\_. Had we all tissue types, we could use facet_wrap(\~Tissue) to create a identical plot.
...
ggplot(data = percent_phyloontogenic,
  ...
  labs(y = "Genes Assigned to Categories (%)",
       x = "Tissue Type")+
  scale_fill_manual(values = c("Accelerated" , "Neotenic"),
                    color = c("Blue", "Green"))
```
